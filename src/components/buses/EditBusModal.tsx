...
import { Route, Bus } from './BusesPage';
...
const EditBusModal: React.FC<EditBusModalProps> = ({ bus, drivers, routes, onClose, onBusUpdated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      customToast.success('Bus updated successfully');
      onBusUpdated();
      onClose();
    } catch (error) {
      customToast.error('Failed to update bus');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-xl">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-gray-900">Edit Bus</h3>
            <button
              onClick={onClose}
